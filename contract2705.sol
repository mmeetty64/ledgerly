...
contract Ledgerly is ERC20("LedgerlyToken", "LED"){
    ...
    enum Role{User, Shop, Admin, Owner}
    ...
    function buyTokens(uint _amount) external payable allWorkRole{
        require(msg.value > 0, unicode"Ошибка введенной суммы");
        require(_amount == msg.value/tokenPrice, unicode"Внесено неправильное количество ether!");
        
        uint256 tokensToBuy = msg.value / (tokenPrice*decim);
        uint256 tokenBalance = balanceOf(Owner);

        require(tokensToBuy <= tokenBalance, unicode"В системе недостаточно токенов");
