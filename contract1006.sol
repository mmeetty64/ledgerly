...
contract Ledgerly is ERC20("LedgerlyToken", "LED"){
    ...
    constructor(){
        ...
        users[0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC] = Role.Admin;
        ...
        shop[_shop] = Shop(unicode"Магазин1", unicode"Наш магазин предлагает широкий ассортимент качественных товаров для всех возрастов и интересов. Мы стремимся предоставить нашим клиентам только лучшие продукты, отобранные с заботой о качестве и уникальности.","logo.jpg", _shop);
        shops.push(Shop(unicode"Магазин1", unicode"Наш магазин предлагает широкий ассортимент качественных товаров для всех возрастов и интересов. Мы стремимся предоставить нашим клиентам только лучшие продукты, отобранные с заботой о качестве и уникальности.", "logo.jpg", _shop));
    ...
    function requireShop(string memory _title, string memory _description, string memory _logo) public onlyUser{
        for (uint i = 0; i < reqShop.length; i++) 
        {
            require(reqShop[i].holder != msg.sender, unicode"Вы уже отправляли заявку!");
        }
    ...
    function viewShopOrders() public view returns(Shop[] memory){
        return reqShop;
    }
